...
class CSPriceChecker:
    ...
    def load_skin_names(self):
        ...
        print(self.name_to_market_hash)
    ...
    def find_price(self, skin_name: str) -> float | None:
        print(f"Получен {skin_name}")
        key = skin_name.lower()
        print(f"Ключ {key}")
        print(self.name_to_market_hash)
        market_hash_name = self.name_to_market_hash.get("ak-47 | midnight laminate (battle-scarred)")
        print(f"Результат {self.name_to_market_hash.get(key)}")
